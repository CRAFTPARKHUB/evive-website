...
export default function SmoothScroll(props) {
    ...
    useEffect(() => {
        ...
        if (overlayElement) {
            ...
            }; // Set up MutationObserver for the html element
...
`;
    ...
    useEffect(() => { // Get all anchor links and store click handlers with their targets
        const anchorLinksData = [...document.querySelectorAll("a[href]")].filter(anchor => anchor.href.includes("#")).map(anchor => {
            const href = `#${anchor.href.split("#").pop()}`;
            const decodedHref = decodeURIComponent(href);
            let scrollMargin = 0;
            const targetElement = document.querySelector(decodedHref);
            if (targetElement) {
                scrollMargin = parseInt(window.getComputedStyle(targetElement).scrollMarginTop);
            }
            return {
                href,
                scrollMargin,
        ...
        const handleClick = (e, href, scrollMargin) => {
            e.preventDefault();
        ...
        const handlers = anchorLinksData.map(({
            href,
            scrollMargin
        }) => e => handleClick(e, href, scrollMargin));
        anchorLinksData.forEach(({
            anchorElement
        }, index) => {
